...
public class WeatherActivityFragment extends Fragment implements WeatherView {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        initRecycler();
        boolean isByCity = getArguments().getBoolean(Constants.BY_CITY);
        if (isByCity) {
            city = getArguments().getString(Constants.CITY_NAME);
            presenter.loadWeather(city);
        } else {
            double lon = getArguments().getDouble(Constants.LONGITUDE,0);
            double lat = getArguments().getDouble(Constants.LATITUDE,0);
            presenter.loadWeather(lon, lat);
        }
        return view;
    }
